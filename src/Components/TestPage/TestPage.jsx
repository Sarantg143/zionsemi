...
const TestPage = () => {
  ...
  useEffect(() => {
    const fetchTestData = async () => {
      try {
        ...
        const degree_courses = localStorage.getItem("degree_courses");
        let testData = {};
        Object.values(JSON.parse(degree_courses)).forEach((course)=>{
          Object.values(course.lessons).forEach((lesson)=>{
            if(lesson.test.test_id===testId){
              testData = lesson.test;
              setLessonID(lesson.name)
            }
          })
        })
  ...
  const calculateScore = () => {
    ...
    setPercentage((correctAnswers / Object.keys(questions).length) * 100);
            ...
            <div className="testPageResultsContainer">
              <h3>
                ...
                <div className="finalScore">
                  <div>
                    <span>Your Score:</span>
                    {score}/{Object.keys(questions).length}
                  </div>
                  <div>
                    <span>Percentage :</span>
                    <span>{percentage}%</span>
            ...
            <>
              ...
              <div className="testPageQuestionBox">
                <div className="testPageTotalQuestion">
                  Question No : {currentQuestionIndex + 1} / {Object.keys(questions).length}
                </div>
